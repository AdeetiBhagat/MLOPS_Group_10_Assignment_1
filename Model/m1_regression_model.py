...
from google.colab import files # only used if running on Google Colab
...
insurance_dataset = files.upload() # if running on Google Colab and if the file is not already uploaded

df_insurance = pd.read_csv(io.BytesIO(insurance_dataset['insurance-1.csv'])) # if running on Google Colab
# df_insurance = pd.read_csv('insurance-1.csv') # if running locally
...
!jupyter nbconvert --to html/content/M1_Regression_Model.ipynb